...
export default function RequestDecisionForm({ id, kind, status }: Props) {
  ...
  return <div className="mt-4 space-y-3 border-t border-gray-200 pt-4">
    {kind === "applications" ? <div className="grid grid-cols-2 gap-3"><input value={applicationFee} onChange={(event) => setApplicationFee(event.target.value)} inputMode="numeric" placeholder="Application fee ($)" className="rounded-xl border border-gray-200 px-3 py-2 text-sm" /><input value={additionalFees} onChange={(event) => setAdditionalFees(event.target.value)} inputMode="numeric" placeholder="Other fees ($)" className="rounded-xl border border-gray-200 px-3 py-2 text-sm" /></div> : <div className="grid grid-cols-2 gap-3"><input type="date" value={confirmedDate} onChange={(event) => setConfirmedDate(event.target.value)} className="rounded-xl border border-gray-200 px-3 py-2 text-sm" /><input type="time" value={confirmedTime} onChange={(event) => setConfirmedTime(event.target.value)} className="rounded-xl border border-gray-200 px-3 py-2 text-sm" /></div>}
    <textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Optional note for the applicant" rows={2} className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm" />
