...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const content = approved
      ? `<p style="font-size:16px;line-height:1.6;">We are pleased to let you know that your rental application has been approved. Please contact your agent to finalize the remaining lease steps.</p>${detailCard([["Property", `${application.propertyTitle} — ${application.propertyAddress}`], ["Application fee", formatCurrency(applicationFee)], ["Security deposit", formatCurrency(additionalFees)], ["Next step", "Contact your Hitts Homes agent to finalize your rental application"]])}${application.decisionNote ? `<p style="font-size:14px;line-height:1.6;color:#475467;"><strong>Note from your agent:</strong> ${escapeHtml(application.decisionNote)}</p>` : ""}`
