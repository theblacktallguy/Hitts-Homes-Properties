...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const propertyCards = properties.map((property) => {
      ...
      return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 16px;border:1px solid #e5e7eb;border-radius:14px;overflow:hidden;background:#ffffff;"><tr><td style="padding:0;"><img src="${propertyImageUrl(property.imageFolder, property.propertyId)}" alt="${escapeHtml(property.title)}" width="100%" style="display:block;width:100%;height:220px;object-fit:cover;background:#f3f4f6;" /></td></tr><tr><td style="padding:18px 18px 10px;"><div style="font-size:11px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:#9A762E;margin-bottom:7px;">${listingType}</div><div style="font-size:20px;line-height:1.25;font-weight:700;color:#0B1F3A;margin-bottom:8px;">${escapeHtml(price)}</div><div style="font-size:15px;line-height:1.4;font-weight:700;color:#162033;margin-bottom:6px;">${escapeHtml(property.title)}</div><div style="font-size:13px;line-height:1.5;color:#667085;margin-bottom:12px;">${escapeHtml(location)}</div><div style="border-top:1px solid #edf0f2;padding-top:11px;font-size:12px;line-height:1.5;color:#475467;">${escapeHtml(details)}</div></td></tr><tr><td style="padding:6px 18px 18px;"><a href="https://hittshomes.com/property/${encodeURIComponent(property.propertyId)}" style="display:block;background:#0B1F3A;border-radius:9px;padding:13px 10px;color:#ffffff;font-size:13px;font-weight:700;text-align:center;text-decoration:none;">View Property Details</a></td></tr></table>`;
    }).join("");
